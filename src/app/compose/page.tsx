      ...
      <div className="grid md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
            <h2 className="font-semibold mb-3 text-zinc-300">Select Recipient</h2>
            <select
              value={selectedId}
            ...
            {selected && (
              <div className="mt-4 p-3 bg-zinc-800 rounded-lg text-sm space-y-1">
                <div><span className="text-zinc-500">Company:</span> <span className="font-medium">{selected.company_name}</span></div>
                <div><span className="text-zinc-500">Email:</span> <span className="text-zinc-300">{selected.email}</span></div>
                {selected.contact_name && <div><span className="text-zinc-500">Contact:</span> <span className="text-zinc-300">{selected.contact_name}</span></div>}
                {selected.emails_sent > 0 && <div className="text-yellow-500 text-xs pt-1">⚠ Already sent {selected.emails_sent} email(s) to this contact</div>}
              </div>
            )}
        ...
        <div className="md:col-span-3 bg-zinc-900 border border-zinc-800 rounded-xl p-5 flex flex-col gap-4">
          <h2 className="font-semibold text-zinc-300">Email Preview & Edit</h2>
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wide mb-1 block">From</label>
            <div className="text-sm text-zinc-400 bg-zinc-800 rounded-lg px-3 py-2">info@monster-design.com</div>
          </div>
          {selected && (
            <div>
              <label className="text-zinc-500 text-xs uppercase tracking-wide mb-1 block">To</label>
              <div className="text-sm text-zinc-400 bg-zinc-800 rounded-lg px-3 py-2">{selected.email}</div>
            </div>
          )}
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wide mb-1 block">Subject</label>
            <input
              value={subject}
          ...
          <div className="flex-1">
            <label className="text-zinc-500 text-xs uppercase tracking-wide mb-1 block">Body</label>
            <textarea
              value={body}
