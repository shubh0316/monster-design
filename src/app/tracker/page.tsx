    ...
    <div>
      ...
      {showLog && (
        <div className="bg-zinc-900 border border-orange-500/40 rounded-xl p-5 mb-6">
          <h2 className="font-semibold mb-4 text-zinc-300">Log Incoming Reply</h2>
          <form onSubmit={logReply} className="space-y-3">
            <div className="grid md:grid-cols-2 gap-3">
              <div>
                <label htmlFor="log-company" className="text-zinc-500 text-xs block mb-1">Company *</label>
              ...
              <div>
                <label htmlFor="log-email" className="text-zinc-500 text-xs block mb-1">Linked to email (optional)</label>
            ...
            <textarea required placeholder="Reply body *" value={logForm.body}
              ...
              rows={5}
              className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500 resize-none" />
      ...
      {followUp && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-bold text-lg">Follow-up Email</h2>
                <p className="text-zinc-500 text-sm">{followUp.row.company_name} · {followUp.row.email}</p>
              </div>
              <button type="button" aria-label="Close" onClick={() => setFollowUp(null)} className="text-zinc-500 hover:text-white text-xl leading-none">✕</button>
            </div>

            {followUp.generating ? (
              <div className="py-10 text-center text-zinc-400 text-sm animate-pulse">Generating follow-up…</div>
            ) : followUp.sent ? (
              <div className="py-10 text-center text-green-400 text-sm">
                Follow-up sent and logged to the tracker.
                <button type="button" onClick={() => setFollowUp(null)} className="block mx-auto mt-4 text-zinc-400 hover:text-white text-xs">Close</button>
              </div>
            ) : (
              <>
                {followUp.error && <div className="bg-red-900/30 border border-red-700 rounded-lg p-3 text-red-300 text-sm">{followUp.error}</div>}
                <div>
                  <label htmlFor="fu-subject" className="text-zinc-500 text-xs block mb-1">Subject</label>
                ...
                <div>
                  <label htmlFor="fu-body" className="text-zinc-500 text-xs block mb-1">Body</label>
                  <textarea id="fu-body" placeholder="Follow-up body" value={followUp.body}
                    onChange={(e) => setFollowUp((f) => f ? { ...f, body: e.target.value } : null)}
                    rows={14}
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500 resize-none font-mono leading-relaxed" />
                </div>
                ...
                <div className="border-t border-zinc-800 pt-3">
                  <p className="text-zinc-600 text-xs">Original sent {daysSince(followUp.row.sent_at)} day(s) ago · &quot;{followUp.row.subject}&quot;</p>
                </div>
              </>
            )}
          </div>
      ...
      {tab === 'sent' ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          {rows.length === 0 ? (
            <div className="p-8 text-zinc-500 text-sm text-center">No emails sent yet. Go to Compose to send your first pitch.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-800">
                  {rows.map((r) => (
                    <tr key={r.sent_email_id} className="hover:bg-zinc-800/50 transition-colors">
                      <td className="px-5 py-3 font-medium">{r.company_name}</td>
                      <td className="px-5 py-3 text-zinc-400">{r.email}</td>
                      <td className="px-5 py-3 text-zinc-300 max-w-xs truncate">{r.subject}</td>
                      <td className="px-5 py-3 text-zinc-500 whitespace-nowrap">
                        <div>{r.sent_at.slice(0, 10)}</div>
                        <div className="text-zinc-600 text-xs">{daysSince(r.sent_at)}d ago</div>
                      </td>
                      <td className="px-5 py-3">
                        <span className={`text-xs font-semibold ${r.reply_count > 0 ? 'text-orange-400' : 'text-zinc-600'}`}>
              ...
              <div key={r.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <span className="text-zinc-500 text-sm ml-2">from {r.from_email}</span>
                  </div>
                  <span className="text-zinc-600 text-xs">{r.received_at.slice(0, 16).replace('T', ' ')}</span>
                </div>
                {r.subject && <div className="text-zinc-400 text-sm mb-2">Re: {r.subject}</div>}
                {r.original_subject && <div className="text-zinc-600 text-xs mb-2">In response to: {r.original_subject}</div>}
                <div className="text-zinc-300 text-sm whitespace-pre-wrap bg-zinc-800 rounded-lg p-3">{r.body}</div>
              </div>
            ))
          )}
        </div>
