...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-zinc-500 text-sm mb-8">Configure your email sending credentials. AI (Groq) and SMTP are pre-configured via environment variables.</p>

      <div className="bg-zinc-900 border border-green-700/40 rounded-xl p-4 mb-6 text-sm">
        <div className="text-green-400 font-medium mb-1">✓ Groq AI configured</div>
        <div className="text-zinc-500 text-xs">Email generation is active using your Groq API key from the environment.</div>
      </div>

      <form onSubmit={save} className="space-y-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-4">
          <h2 className="font-semibold text-zinc-300">Email — SMTP</h2>
          <p className="text-zinc-500 text-sm">Emails will be sent from <span className="text-orange-400">info@monster-design.com</span>. Enter your SMTP credentials below.</p>
          {field('smtp_host', 'SMTP Host', 'text', 'e.g. smtp.gmail.com or mail.monster-design.com')}
          <div className="grid grid-cols-2 gap-3">
            {field('smtp_port', 'SMTP Port', 'text', '587')}
            <div>
              <label className="text-zinc-400 text-sm block mb-1">Secure (SSL/TLS)</label>
          ...
          <div className="bg-zinc-800 rounded-lg p-3 text-xs text-zinc-500 space-y-1">
            <div className="text-zinc-400 font-medium mb-1">Common setups:</div>
